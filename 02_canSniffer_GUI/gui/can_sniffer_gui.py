...
LABEL_DICT_PATH = "save/labelDict.csv"
DECODED_PACKETS_PATH = "save/decodedPackets.csv"
...
class CanSnifferGUI(QMainWindow, canSniffer_ui.Ui_MainWindow):
    def __init__(self):
        ...
        self.decodedMessagesTableWidget.horizontalHeader().setSectionResizeMode(
    ...
    def playbackMainTable1Packet(self):
        ...
        txBuf += (
            id
            + ","
            + self.mainMessageTableWidget.item(row, 2).text()
            + ","
    ...
    def sendDecodedPacketCallback(self):
        ...
        newId = str(
            self.decodedMessagesTableWidget.item(decodedCurrentRow, 1).text()
        ).split(" ")
        newItem = QTableWidgetItem(newId[0])
        self.txTable.setItem(newRow, 0, QTableWidgetItem(newItem))
        for i in range(1, 3):
            self.txTable.setItem(
                newRow,
                i,
                QTableWidgetItem(
                    self.decodedMessagesTableWidget.item(decodedCurrentRow, i + 1)
                ),
            )
        newData = ""
        for i in range(
            int(self.decodedMessagesTableWidget.item(decodedCurrentRow, 4).text())
        ):
            newData += str(
                self.decodedMessagesTableWidget.item(decodedCurrentRow, 5 + i).text()
            )
    ...
    def decodedTableItemChangedCallback(self):
        if self.isInited:
            self.saveTableToFile(
                self.decodedMessagesTableWidget, "../save/decodedPackets.csv"
            )

    def deleteDecodedLineCallback(self):
        self.decodedMessagesTableWidget.removeRow(
            self.decodedMessagesTableWidget.currentRow()
        )
    ...
    def addToDecodedCallback(self):
        ...
        for i in range(1, self.decodedMessagesTableWidget.columnCount()):
            new_item = QTableWidgetItem(
                self.mainMessageTableWidget.item(
                    self.mainMessageTableWidget.currentRow(), i
                )
            )
        ...
        for row in range(self.txTable.rowCount()):
            if self.txTable.item(row, 0).isSelected():
                ...
                if self.repeatedDelayCheckBox.isChecked():
                    self.serialWriterThread.setRepeatedWriteDelay(
                        self.repeatTxDelayValue.value()
                    )
                else:
                    self.serialWriterThread.setRepeatedWriteDelay(0)
                self.serialWriterThread.write(txBuf)
    ...
    def init(self):
        self.loadTableFromFile(self.decodedMessagesTableWidget, DECODED_PACKETS_PATH)
        self.loadTableFromFile(self.idLabelDictTable, LABEL_DICT_PATH)
        for row in range(self.idLabelDictTable.rowCount()):
            self.idLabelDict[str(self.idLabelDictTable.item(row, 0).text())] = str(
                self.idLabelDictTable.item(row, 1).text()
            )
        self.isInited = True

    def clearLabelDict(self):
        self.idLabelDictTable.setRowCount(0)
        self.saveTableToFile(self.idLabelDictTable, LABEL_DICT_PATH)

    def saveTableToFile(self, table, path):
        if path is None:
            path, _ = QFileDialog.getSaveFileName(
                self, "Save File", "./save", "CSV(*.csv)"
            )
        if path != "":
                ...
                for row in range(table.rowCount() - 1, -1, -1):
                    ...
                    for column in range(table.columnCount()):
                        ...
                        if item is not None:
                            tempItem = item.text()
                            if (
                                self.exportDecodedListInMillisecTimestamp
                                and column == 0
                            ):
    ...
    def mainTablePopulatorCallback(self, rowData):
        ...
        if self.showOnlyIdsCheckBox.isChecked():
            if newId not in self.showOnlyIdsSet:
                return
        if self.hideIdsCheckBox.isChecked():
            if newId in self.hideIdsSet:
                return
        ...
        for i in range(self.mainMessageTableWidget.columnCount()):
            if i < len(rowData):
                ...
                if item:
                    if item.text() != data:
                        if (
                            self.highlightNewDataCheckBox.isChecked()
                            and self.groupModeCheckBox.isChecked()
                            and i > 4
                        ):
                            newItem.setBackground(QColor(104, 37, 98))  # purple
                else:
                    if (
                        self.highlightNewDataCheckBox.isChecked()
                        and self.groupModeCheckBox.isChecked()
        ...
        if self.highlightNewIdCheckBox.isChecked():
            if newId not in self.idDict.keys():
                for j in range(3):
                    self.mainMessageTableWidget.item(row, j).setBackground(
                        QColor(52, 44, 124)
                    )  # dark blue
        ...
        for i in range(self.mainMessageTableWidget.columnCount()):
            if (isFamiliar or (newId.find("(") >= 0)) and i < 3:
                self.mainMessageTableWidget.item(row, i).setBackground(
                    QColor(53, 81, 52)
                )  # dark green

            self.mainMessageTableWidget.item(row, i).setTextAlignment(
                    ...
                    for rowData in csv.reader(stream):
                        ...
                        for i in range(len(rowData)):
                            if len(rowData[i]):
                                ...
                                if not (
                                    table == self.decodedMessagesTableWidget and i == 0
                                ):
    ...
    def cellWasClicked(self):
        self.saveIdToDictLineEdit.setText(
            self.mainMessageTableWidget.item(
                self.mainMessageTableWidget.currentRow(), 1
            ).text()
        )

    def saveIdLabelToDictCallback(self):
        if (not self.saveIdToDictLineEdit.text()) or (
            not self.saveLabelToDictLineEdit.text()
        ):
            return
        ...
        self.idLabelDict[str(self.saveIdToDictLineEdit.text())] = str(
            self.saveLabelToDictLineEdit.text()
        )
        self.saveIdToDictLineEdit.setText("")
    ...
    def startSniffing(self):
        ...
        if self.activeChannelComboBox.isEnabled():
            ...
            txBuf = [
                0x41,
                1 << self.activeChannelComboBox.currentIndex(),
            ]  # RX FORWARDER
            self.serialWriterThread.write(txBuf)
    ...
    def serialPacketReceiverCallback(self, packet, time):
        ...
        if DLC > 0:
            rowData += [
                packetSplit[3][i : i + 2] for i in range(0, len(packetSplit[3]), 2)
            ]  # value
